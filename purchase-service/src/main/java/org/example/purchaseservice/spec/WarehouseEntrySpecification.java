...
public class WarehouseEntrySpecification implements Specification<WarehouseEntry> {
    ...
    @Override
    public Predicate toPredicate(@NotNull Root<WarehouseEntry> root, CriteriaQuery<?> query,
            ...
            if (filters.containsKey("entry_date_to")) {
                ...
                if (!toDates.isEmpty()) {
                    try {
                        ...
                        throw new WarehouseException(String.format("IInvalid entry_date_to format: %s",
                                toDates.getFirst()));
                    }
                }
            }
