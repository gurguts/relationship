package org.example.statusclientservice.restControllers;

import jakarta.validation.Valid;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.example.statusclientservice.mappers.StatusClientMapper;
...
public class StatusClientController {
    ...
    @PostMapping
    public ResponseEntity<StatusClientDTO> createStatusClient(@RequestBody @Valid StatusClientCreateDTO statusClientCreateDTO) {
        StatusClient statusClient = statusClientMapper.statusClientCreateDTOToStatusClient(statusClientCreateDTO);
        StatusClientDTO createdStatusClient = statusClientMapper.statusClientToStatusClientDTO(statusClientService.createStatusClient(statusClient));
    ...
    @PreAuthorize("hasAuthority('settings:edit')")
    @PutMapping("/{id}")
    public ResponseEntity<StatusClientDTO> updateStatusClient(@PathVariable Long id, @RequestBody @Valid StatusClientUpdateDTO statusClientUpdateDTO) {
    ...
    public ResponseEntity<List<StatusClientDTO>> findByNameContaining(@RequestParam String query) {
        return ResponseEntity.ok(statusClientService.findByNameContaining(query).stream().map(statusClientMapper::statusClientToStatusClientDTO).toList());
    }
}
