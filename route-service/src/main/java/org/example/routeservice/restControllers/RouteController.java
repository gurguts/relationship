package org.example.routeservice.restControllers;

import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.example.routeservice.mappers.RouteMapper;
...
public class RouteController {
    ...
    public ResponseEntity<List<RouteDTO>> findByNameContaining(@RequestParam String query) {
        return ResponseEntity.ok(routeService.findByNameContaining(query).stream().map(routeMapper::routeToRouteDTO).toList());
    }
}
