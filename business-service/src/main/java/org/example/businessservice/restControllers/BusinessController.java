package org.example.businessservice.restControllers;

import jakarta.validation.Valid;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.example.businessservice.mappers.BusinessMapper;
...
public class BusinessController {
    ...
    @PreAuthorize("hasAuthority('settings:edit')")
    @PutMapping("/{id}")
    public ResponseEntity<BusinessDTO> updateBusiness(@PathVariable Long id, @RequestBody @Valid BusinessUpdateDTO businessUpdateDTO) {
    ...
    public ResponseEntity<List<BusinessDTO>> findByNameContaining(@RequestParam String query) {
        return ResponseEntity.ok(businessService.findByNameContaining(query).stream().map(businessMapper::businessToBusinessDTO).toList());
    }
}
