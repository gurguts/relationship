package org.example.saleservice.spec;
...
public class SaleSpecification implements Specification<Sale> {
    ...
    public SaleSpecification(String query, Map<String, List<String>> filterParams,
    ...
    @Override
    public Predicate toPredicate(@NonNull Root<Sale> root, CriteriaQuery<?> query,
    ...
    private Predicate applyFilters(Predicate predicate, Root<Sale> root, CriteriaBuilder criteriaBuilder) {
    ...
    private Predicate addIdFilter(Predicate predicate, Root<Sale> root, CriteriaBuilder criteriaBuilder,
    ...
    private Predicate addStringFilter(Predicate predicate, Root<Sale> root, CriteriaBuilder criteriaBuilder,
                                      List<String> values, String field) {
        if (!values.isEmpty()) {
    ...
    private Predicate addPaymentMethodFilter(Predicate predicate, Root<Sale> root, CriteriaBuilder criteriaBuilder,
                                             List<String> values) {
        if (!values.isEmpty()) {
    ...
    private Predicate addDateFilter(Predicate predicate, Root<Sale> root, CriteriaBuilder criteriaBuilder,
    ...
    private Predicate addNumericFilter(Predicate predicate, Root<Sale> root, CriteriaBuilder criteriaBuilder,
                                       List<String> values, String field, boolean isFrom) {
        if (!values.isEmpty()) {
