...
import org.example.clientservice.mappers.field.RegionMapper;
...
public class RegionController {
    ...
    public ResponseEntity<List<RegionDTO>> findByNameContaining(@RequestParam String query) {
        return ResponseEntity.ok(regionService.findByNameContaining(query).stream().map(
                regionMapper::regionToRegionDTO).toList());
    }
}
